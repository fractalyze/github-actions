...
def get_bot_comments(repo: str, pr_number: int) -> list[dict]:
    """Get all comments with pr-reviewer-bot signature."""
    comments = []

    # PR review comments (inline on diff)
    pr_comments = gh_api(f"repos/{repo}/pulls/{pr_number}/comments") or []
    for c in pr_comments:
        if BOT_SIGNATURE in c.get("body", ""):
            comments.append({"id": c["id"], "type": "pr"})

    # Issue comments (general PR conversation)
    issue_comments = gh_api(f"repos/{repo}/issues/{pr_number}/comments") or []
...
def update_metrics(metrics_dir: Path, repo: str, pr_number: int, reactions: dict, score: float):
    ...
    print(f"Updated: score={round(score, 2)}, overall={round(overall, 1)} ({total_prs} PRs)")


def main():
    repo = os.environ.get("REPO")
    pr_number = int(os.environ.get("PR_NUMBER", 0))
    metrics_dir = Path(os.environ.get("METRICS_DIR", "dot-claude/agents/pr-reviewer/metrics"))

    if not repo or not pr_number:
        print("Error: REPO and PR_NUMBER environment variables required")
        return 1
    ...
    if not comments:
        print("No pr-reviewer-bot comments found. Skipping.")
        return 0
